...
class EnhancedBookDetailScreen extends StatefulWidget {
  ...
  @override
  _EnhancedBookDetailScreenState createState() => _EnhancedBookDetailScreenState();
}
...
class _EnhancedBookDetailScreenState extends State<EnhancedBookDetailScreen>
  ...
  Widget _buildErrorScreen() {
    return Scaffold(
      ...
      backgroundColor: Color(0xFF1A1A2E),
      body: Center(child: Text(_error ?? 'Error', style: TextStyle(color: Colors.white))),
    );
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        GridView.count(
                          ...
                          mainAxisSpacing: 12,
                          children: [
                            _buildInfoCard('Gutendex ID', book.id.toString(), Icons.tag), // Fix: book.id to string
                            _buildInfoCard('Downloads', book.getFormattedDownloads(), Icons.download),
                            _buildInfoCard(
                              'Language', 
                              book.languages.isNotEmpty ? book.languages.join(', ').toUpperCase() : '-', 
                              Icons.language
                            ),
                            // Fix: 'format' tidak ada di model, kita hardcode 'EPUB' jika readable
                            _buildInfoCard('Format', book.isReadable ? 'EPUB' : 'Unknown', Icons.file_present),
                          ],
                        ),
                        
                        SizedBox(height: 24),
                        Text('Subjects / Shelves', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: Colors.white)),
                        SizedBox(height: 12),
                        Text(
                          book.description,
                          ...
                          child: Material(
                            ...
                            child: InkWell(
                              ...
                              child: Row(
                                ...
                                children: [
                                  Icon(
                                    book.isReadable ? Icons.menu_book_rounded : Icons.lock_outline,
                                    color: Colors.white.withOpacity(book.isReadable ? 1.0 : 0.5),
                                  ),
                                  SizedBox(width: 8),
                                  Text(
                                    book.isReadable 
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            SizedBox(width: 6),
            Expanded(child: Text(title, style: TextStyle(fontSize: 10, color: Colors.white54), overflow: TextOverflow.ellipsis)),
          ]),
          SizedBox(height: 4),
          Text(value, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.white), overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
